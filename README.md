...
- **Agendas, Static & Dynamic:** Type an optional agenda when you start a meeting (comma-separated). The Live Brief panel shows a live **coverage checklist** that ticks items off as they're discussed — and *also surfaces topics the meeting itself raised* that you didn't list (marked with a ✨), so the agenda builds itself even when you start with a blank one. The final agenda — planned items and discovered ones, each checked or not — is written into the notes as its own **`# Agenda`** checklist.
- **Ask Before It Ends:** When you deliberately end a meeting, GhostWriter does a quick coverage check and — if anything's still open (agenda items you skipped, or topics raised but left unresolved) — offers **Keep Recording / End Anyway** with the list, so nothing important slips through. Cloud-only and best-effort; skipped for very short meetings and in Local-only mode.
...
- **In-App Notes Viewer/Editor:** Open any note in a built-in Markdown editor — edit and save, **export to a formatted PDF**, or jump out via "Open in Default App" / "Reveal in Finder". Reachable from the menu, the Notes Assistant, and search/ask results.
...
| `Sources/NotesViewerWindow.swift` | In-app Markdown viewer/editor (edit, follow-up, rename, PDF export, open externally) |
