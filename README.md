...
- **Ask Anything** — a multi-turn chat across your whole knowledge base, scoped to all meetings, a chosen set, or an org/project, with cited sources. In agentic mode it plans several searches per question and folds in Catalog facts (accounts, opportunities, POC health, people), so it answers structured questions ("which POCs are at risk?") as well as "what did we decide with Acme?". (Browsing, text/semantic search, and action-item export to Reminders live in the Catalog and Digest.)
