...
- **Catalog:** A lightweight CRM-style organiser that sits *beside* your notes without touching them — a graph of the **organisations, people, projects, opportunities and tags** your meetings are about. Open it from the menu (**Catalog…**). A three-column browser (sections → list → editor) plus a **Map** tab that renders the whole thing as one expandable, searchable tree you can pick from. Organisations form an **unlimited hierarchy** (each with an editable relationship: Root / Customer / Prospect / Partner / Internal / Other); projects belong to orgs, and opportunities to projects. **Notes are assigned to an opportunity *or* an organisation** (mutually exclusive) — everything else (project → org → people) is **inherited automatically** up the chain. **Import notes** pulls in your meeting files (`Meeting_*.md` only); each note's editor shows its inherited links, **tags** (with suggestions promoted from the note's own front-matter — a note token can be turned into any entity), and its **action items** (tick + export to Reminders, individually or all at once). Filter and search notes from the toolbar — **Text**, on-device **Meaning** (semantic), and **Ask** (a cloud Q&A scoped to the filtered notes, with cited sources that open in the editor). **Quick add** builds a whole org → project → opportunity → people → tags chain (with a parent) in one sheet. Stored as a single `Catalog.json` in the notes folder — rebuildable, backup-friendly, with a **Purge** action that clears the catalog while leaving your note files intact.
...
| `Sources/Catalog.swift` | Catalog model + `CatalogStore` (Codable `Catalog.json` store: orgs/people/projects/opportunities/tags/note-links, hierarchy, inheritance, import, purge) |
| `Sources/CatalogWindow.swift` | Catalog window — three-column browser, Map tree, note linking, toolbar search (Text/Meaning/Ask) + filters, Quick add |
