...
| `Sources/Views/CatalogDashboard.swift` | Catalog **Dashboard** (Overview section) — SE-oriented insight cards (POC command center, relationships, activity, **meeting-type mix** funnel — Discovery → Demo → Scoping → Kickoff, action items, competitive/product intelligence, open technical questions) + an "At a glance" KPI strip that shows the global catalog snapshot and re-scopes to the filtered set once a range/account is chosen. Time range spans Today → All time. Uses Swift Charts. Aggregation reuses `DigestService`, `NotesLibrary.actionItems`, and `RadarInsights` |
...
| `Sources/Meetings/FollowUpPacket.swift` | One-click **Follow-Up Packet** — assembles a client follow-up email, an updated POC plan (grounded in the linked opportunity's current success criteria), and the meeting's action items into one Markdown document. AI sections run concurrently and degrade individually; action items reuse the note's curated `## Action Items` when present. Per-section toggles in Settings; opened via the notes viewer's Draft… menu |
