...
Got a recording — a voice note from a chat app, a call export? Choose **Transcribe Audio File…** from the menu bar (or **drag files onto the Catalog window**) to open the **Transcribe Audio** window. Drop or browse for files, optionally pick an **org/project** to file them under, and hit **Transcribe** — a per-file progress list shows each one working → done, and a **completion banner** confirms the result (with a **View in History** button). Each becomes a **meeting note with an AI title, dated to the file's own recording time**, added to the Catalog. The window has two tabs (a segmented control): **Transcribe** (drop zone + progress) and **History**. Finished files drop out of the queue into the **History** tab (with a title filter); each history row's **⋯ menu** offers Open in Viewer, **Reveal in Catalog**, Reveal in Finder, Copy Transcript, Regenerate from Audio, Clear from History (just hides the row), and **Move to Trash…** (deletes the note file, recoverable). A file you already transcribed is flagged **Already transcribed** and skipped by default — its menu offers **Transcribe anyway**, **Show in History**, or (if you'd cleared it) **Add to History** to put it back for free. The History tab's ⋯ menu has **Reload**, **Rebuild from Notes** (rescans and re-lists every transcribed note — brings back anything you cleared), and **Clear All from History**. From the Catalog you can also relink a note, **Move to Dictation** if it turned out to be a plain voice memo rather than a meeting, or delete it. Supports wav, mp3, m4a, ogg/opus, flac, and webm; the size limit is set in Settings → Dictation → Audio Import (default 50 MB).
...
- **Recent Notes ▸** — a quick-jump opener: the current/latest meeting notes (⌃⌥N), today's quick notes, the last 5 meetings grouped by day, a **Browse All Notes in Catalog…** link that opens the Catalog straight to its Notes section, **Identify Speakers…**, and the notes folder. Full browsing and search live in the **Catalog** — this submenu just links out to it, so there's a single canonical browser. In the note viewer, the **Draft…** menu leads with a one-click **Follow-Up Packet** — a follow-up email, updated POC plan, and action items in a single document, exportable as one PDF (choose and reorder its sections in Settings → Meetings → Draft Templates).
