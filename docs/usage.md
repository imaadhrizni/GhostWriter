...
- **Catalog…** — organise notes into a graph of organisations, people, projects, opportunities and tags; browse by section or the Map tree, filter/search/Ask, and Quick-add a whole chain. Also home to the **POC Tracker** (Tools section): pick an opportunity and track its proof-of-concept success criteria — add them by hand or **Suggest from meetings** to pull them from the opportunity's linked notes, then cycle each Pending → Passed → Failed as the POC progresses.
